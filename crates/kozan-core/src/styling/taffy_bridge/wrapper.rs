...
mod tests {
    ...
    fn styled_cv(inline_style: &str) -> servo_arc::Arc<ComputedValues> {
        ...
        div.set_attribute("style", inline_style);
        doc.root().append(div);
        doc.recalc_styles();
        doc.computed_style(div.handle().raw().index())
            .expect("recalc must produce ComputedValues")
    }

    #[test]
    fn block_generation_mode_is_normal() {
        let cv = styled_cv("display: block;");
        let wrapper = TaffyStyloStyle(cv);
        assert_eq!(
            wrapper.box_generation_mode(),
            taffy::BoxGenerationMode::Normal,
            "display:block must produce Normal box generation mode",
        );
    }

    #[test]
    fn display_none_generation_mode_is_none() {
        let cv = styled_cv("display: none;");
        let wrapper = TaffyStyloStyle(cv);
        assert_eq!(
            wrapper.box_generation_mode(),
            taffy::BoxGenerationMode::None,
            "display:none must produce None box generation mode",
        );
    }
    ...
    #[test]
    fn from_arc_produces_valid_wrapper() {
        let cv = styled_cv("display: block;");
