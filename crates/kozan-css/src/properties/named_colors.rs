//! CSS named color table — sorted for zero-alloc binary search.

use kozan_style::AbsoluteColor;

/// Looks up a CSS named color (case-insensitive, zero allocation).
pub(crate) fn lookup(name: &str) -> Option<AbsoluteColor> {
    NAMED_COLORS
        .binary_search_by(|(key, _)| {
            // Table keys are lowercase. Compare by lowercasing input bytes on the fly.
            key.as_bytes().iter().copied()
                .cmp(name.as_bytes().iter().map(|b| b.to_ascii_lowercase()))
        })
        .ok()
        .map(|i| AbsoluteColor::from_hex(NAMED_COLORS[i].1))
}

/// All 148 CSS named colors + transparent, sorted for binary search.
